...
void LCDNokia_init(void) {
	...
	gpio_pin_config_t pin_config_gpio =
		{ kGPIO_DigitalOutput, DATA_OR_CMD_PIN };

	gpio_pin_config_t pin_config_gpioB =
			{ kGPIO_DigitalOutput, BIT9 };

	gpio_pin_config_t pin_config_gpioD =
			{ kGPIO_DigitalOutput, RESET_PIN };

	CLOCK_EnableClock(kCLOCK_PortD);
	PORT_SetPinConfig(PORTD, BIT3, &config_pin);
	GPIO_PinInit(GPIOD, BIT3, &pin_config_gpio); //data direction

	PORT_SetPinConfig(PORTD, RESET_PIN, &config_pin);
	GPIO_PinInit(GPIOD, RESET_PIN, &pin_config_gpioD);
	//Configure control pins

	CLOCK_EnableClock(kCLOCK_PortB);
	PORT_SetPinConfig(PORTB, BIT4, &config_pin);
	GPIO_PinInit(GPIOB, BIT9, &pin_config_gpioB);

	/////////////////////////////////////////////
	GPIO_ClearPinsOutput(GPIOB,(BIT1<<BIT9));
	GPIO_SetPinsOutput(GPIOB, (BIT1<<BIT9));


	GPIO_ClearPinsOutput(GPIOD, (BIT1<<RESET_PIN));
	LCD_delay();
	GPIO_SetPinsOutput(GPIOD, (BIT1<<RESET_PIN));
...
void LCDNokia_bitmap(const uint8_t* my_array){
	uint16_t index=0;
	for (index = 0 ; index < (LCD_X * LCD_Y / BIT8) ; index++)
...
{
	if(DataOrCmd)
		GPIO_SetPinsOutput(GPIOD, (BIT1<<DATA_OR_CMD_PIN));
	else
